...
public class GuessingQuestUI extends JFrame {
  ...
  public GuessingQuestUI(GamePanel gp) {
    this.gp = gp;
    ...
    gp.addMouseListener(new MouseAdapter() {
      public void mousePressed(MouseEvent e) {
        ...
        if (noButton.contains(e.getPoint())) {
          visible = false;
          gp.gameState = 1;
          gp.player.movementHandler.ZoroDialogShown = false;
        }

        if (answer1.contains(e.getPoint())) {
          gp.questManager.guessingQuest.choose(0);
        }

        if (answer2.contains(e.getPoint())) {
          gp.questManager.guessingQuest.choose(1);
        }

        if (answer3.contains(e.getPoint())) {
          gp.questManager.guessingQuest.choose(2);
  ...
  public void draw(Graphics2D g2) {
    ...
    if (accepted) {
      ...
      if (showingStory) {
        ...
        g2.setColor(new Color(255, 80, 170));
        g2.setFont(new Font("Arial", Font.BOLD, 16));
        g2.drawString("NA PŘEČTENÍ HÁDANKY MÁŠ 45 SEKUND!", x + 23, y + 25);

        g2.setColor(Color.WHITE);
        g2.setFont(new Font("Arial", Font.PLAIN, 18));

        int q = questionIndex;
        String story = gp.questManager.guessingQuest.stories[q];
      ...
      if (!showingStory) {
        ...
        int q = questionIndex;
        String text = gp.questManager.guessingQuest.questions[q];
        g2.setColor(new Color(255, 80, 170));
        g2.setFont(new Font("Arial", Font.BOLD, 16));
        ...
        if (questionIndex == 0) {
          g2.drawString("A)Slunce", answer1.x, answer1.y + 45);
          g2.drawString("B)Měsíc", answer2.x + 5, answer2.y + 45);
          g2.drawString("C)Hvězda", answer3.x - 15, answer3.y + 45);
        }


        if (questionIndex == 1) {
          g2.drawString("A)Liška", answer1.x, answer1.y + 45);
          g2.drawString("B)Králík", answer2.x - 5, answer2.y + 45);
          g2.drawString("C)Pes", answer3.x - 30, answer3.y + 45);
        }
      }

      if (questionIndex == 2) {
        g2.drawString("A)Déšť", answer1.x - 10, answer1.y + 45);
        g2.drawString("B)Sníh", answer2.x - 10, answer2.y + 45);
        g2.drawString("C)Mrak", answer3.x - 20, answer3.y + 45);
      }
    }
  }
